...
function sanitizeMongoConnections(connections) {
  const cleaned = Object.create(null);
  if (!connections || typeof connections !== 'object') return cleaned;
  for (const [id, value] of Object.entries(connections)) {
    if (!isValidMongoConnectionId(id)) continue;
...
function readSettings() {
  try {
    ...
    return { connections: sanitizeMongoConnections(data.connections) };
  } catch {
    return { connections: {} };
  }
}
...
function writeSettings(data) {
  ...
  const payload = encryptJson({ connections: sanitizeMongoConnections(data.connections) });
...
function createTestServer() {
  return http.createServer(async (req, res) => {
    ...
    if (req.method === 'POST' && pathname === '/api/mongo/connections') {
      ...
      const { id: rawId, name, uri: rawUri, database, authMode } = body || {};
      ...
      if (!isValidMongoConnectionId(id)) {
      ...
      if (!validateMongoUri(uri)) {
        return sendJson(res, 400, { error: 'uri must use the mongodb:// or mongodb+srv:// scheme' });
      }
    ...
    if (req.method === 'DELETE' && deleteMatch) {
      ...
      const settings = readSettings();
      if (Object.prototype.hasOwnProperty.call(settings.connections, id)) delete settings.connections[id];
      writeSettings(settings);
      return sendJson(res, 200, { ok: true });
    }
    ...
    if (req.method === 'POST' && testMatch) {
      ...
      const settings = readSettings();
      const conn = Object.prototype.hasOwnProperty.call(settings.connections, id)
        ? settings.connections[id]
        : null;
...
test('POST /api/mongo/connections rejects reserved id __proto__', async () => {
  const res = await httpRequest(server, 'POST', '/api/mongo/connections', {
    id: '__proto__',
    uri: 'mongodb://localhost:27017',
  });
...
test('POST /api/mongo/connections rejects non-mongodb URI schemes', async () => {
  const res = await httpRequest(server, 'POST', '/api/mongo/connections', {
    id: 'bad-uri',
...
test('DELETE /api/mongo/connections/:id rejects invalid id', async () => {
  const del = await httpRequest(server, 'DELETE', '/api/mongo/connections/bad.id');
  assert.equal(del.status, 400);
  assert.match(del.body.error, /invalid connection id/i);
});
...
test('POST /api/mongo/connections/:id/test rejects invalid id', async () => {
  const res = await httpRequest(server, 'POST', '/api/mongo/connections/bad.id/test');
  assert.equal(res.status, 400);
