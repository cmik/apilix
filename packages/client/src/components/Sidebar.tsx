...
const ConnectionEditorModal = lazy(() => import('./DatabasePanel').then((m) => ({ default: m.ConnectionEditorModal })));
...
export default function Sidebar() {
  ...
  const handleEditDatabase = (conn: any) => {
    ...
    setShowDatabaseEditor(true);
  };

  const handleSaveDatabase = (conn: any) => {
    if (editingDatabase) {
          ...
          {showDatabaseEditor && (
            <Suspense fallback={null}>
              <ConnectionEditorModal
                initial={editingDatabase}
                runtimeVars={{ ...state.globalVariables, ...(state.activeEnvironmentId ? (state.environments.find(e => e._id === state.activeEnvironmentId)?.values ?? []).reduce((acc: any, v: any) => ({ ...acc, [v.key]: v.value }), {}) : {}) }}
                onSave={handleSaveDatabase}
                onClose={() => { setShowDatabaseEditor(false); setEditingDatabase(undefined); }}
              />
            </Suspense>
          )}
        </div>
